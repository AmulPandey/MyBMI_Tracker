...
import com.example.bmi.utils.displayToast
import com.example.bmi.utils.saveBitmap
...
class Result : AppCompatActivity() {
    ...
    private var newWeight: Double = 0.0
    private var newBmi: Double = 0.0
    ...
    private fun saveIntoChart() {
        if (newWeight > 0 && newBmi > 0) {
            lifecycleScope.launch(Dispatchers.IO) {
                ...
                if (currentUser != null) {
                    ...
                    val updatedWeightHistory = (currentUser.weightHistory + newWeight.toFloat()).takeLast(7)
                    val updatedBmiHistory = (currentUser.bmiHistory + newBmi.toFloat()).takeLast(7)
                    userDao.updateUserDetails(currentUser.id, updatedWeightHistory, updatedBmiHistory)
                } else {
                    // Create new user
                    val newUser = User(
                        date = System.currentTimeMillis(),
                        weightHistory = listOf(newWeight.toFloat()),
                        bmiHistory = listOf(newBmi.toFloat())
                    )
            ...
            Toast.makeText(this@Result, "Something Went Wrong", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun bmiCal() {
        if (height > 0 && weight > 0) {
            ...
            // Assign global variables after calculation
            newWeight = weight
            newBmi = result
            showResult()
        }
